...
from model.users import UserModel, TokenModel
...
security = HTTPBearer(auto_error=False)  # for 403 instead of 401 problem in FastAPI
...
def decode_refresh_token(token):  # bug
    try:
        decoded = jwt.decode(token, settings.JWT_SECRET_KEY, algorithms="HS256")
        user_id = decoded.get("user_id", None)
        if not user_id:
